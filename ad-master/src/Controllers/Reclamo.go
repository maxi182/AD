		...
		fmt.Println(c.Request.URL.Query())
		 ...
		 limit, _ := strconv.Atoi(c.DefaultQuery("limit", "100"))
		  ...
		  paginator := pagination.Paging(&pagination.Param{
			DB:   Config.DB.Model(&rec).Preload("Comentarios").Preload("Usuario").Preload("Comentarios.Usuario").Preload("Comentarios.Fotos").Preload("Propiedad").Preload("SharedAreas").Select("*").Joins("inner join Comentarios on Comentarios.reclamo_id = Reclamos.id").Find(&rec),
			Page:    page,
			Limit:   limit,
...
func getQueryReclamo(reclamo *[]Models.Reclamo, userId string, propId string)  *gorm.DB{
	...
	query:= Config.DB.Model(&reclamo).Preload("Comentarios").Preload("Usuario").Preload("Comentarios.Usuario").Preload("Comentarios.Fotos").Preload("Propiedad").Preload("Unidad").Preload("SharedAreas").Select("*").Joins("inner join Usuarios on Reclamos.usuario_id = Usuarios.id").Find(&reclamo)
